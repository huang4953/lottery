...
/**
 * 调用process完成排列三机选单式一注的投注，返回投注结果
 * 返回的字符串为sms.properties中配置的命令
 * @author bing.li
 *
 */
public class SMSService3 extends SMSService{

	private Log logger = LogFactory.getLog(SMSService3.class.getName());
	...
	public String process(String cmd, MO mo){
		...
		try{
			...
			if(!user.isBindMobileFlag()){
				...
				String betCode = LotteryTools.lotteryRandomBetCode(1000003);
				BetServiceInterf betService = ApplicationContextUtils.getService("busLotteryBetService", BetServiceInterf.class);
				String betResult = betService.betSms(user.getUserId(), 1000003, 0, 0, 1, betCode);
				content = AutoProperties.getString(cmd + ".000000", "恭喜您，投注成功：", resource);
				String strs[] = betResult.split("#");
				logger.debug("content=" + content + ", strs=" + strs.toString() + ", index1=" + content.indexOf("{lotteryTerm}") + ", index2=" + content.indexOf("{planNo}"));
	...
	public static void main(String[] args){
		String s = AutoProperties.getString("3.000000", "恭喜您，投注成功：", "com.success.lottery.sms.sms");
		System.out.println(s.indexOf("{lotteryTerm}"));
		System.out.println(s.indexOf("{planNo}"));
		System.out.println(s.replaceAll("\\{lotteryTerm\\}", "10081"));
