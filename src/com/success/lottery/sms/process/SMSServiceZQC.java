 ...
 *
 */
public class SMSServiceZQC extends SMSService{

	private Log logger = LogFactory.getLog(SMSServiceZQC.class.getName());
		...
		try{
			...
			if(!user.isBindMobileFlag()){
				...
				if(parameters.length < 3){
					...
					if("11".equalsIgnoreCase(parameters[1].trim())){
						//胜平负
						lotteryId = 1300001;
				...
				if(content.indexOf("{lotteryId}") >= 0){
					content = content.replaceAll("\\{lotteryId\\}", "" + LotteryTools.getLotteryName(lotteryId));
				}
