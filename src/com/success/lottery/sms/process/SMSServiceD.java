...
/**
 * 调用process完成大乐透机选单式一注的投注，返回投注结果
 * 返回的字符串为sms.properties中配置的命令
 * @author bing.li
 *
 */
public class SMSServiceD extends SMSService{

	private Log logger = LogFactory.getLog(SMSServiceD.class.getName());
	...
	public String process(String cmd, MO mo){
		...
		try{
			...
			if(!user.isBindMobileFlag()){
				...
				String betCode = LotteryTools.lotteryRandomBetCode(1000001);
				BetServiceInterf betService = ApplicationContextUtils.getService("busLotteryBetService", BetServiceInterf.class);
				logger.debug("betService=" + betService + ", user=" + user + ", betCode=" + betCode);
				String betResult = betService.betSms(user.getUserId(), 1000001, 0, 0, 1, betCode);
