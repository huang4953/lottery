 ...
 *
 */
public class SMSService0000 extends SMSService{
	...
	public String process(String cmd, MO mo){
		...
		String moRs = null;
		String mtRs = null;
		String content = AutoProperties.getString(cmd + ".000099", "您并未订购任何上海唐路的业务！", resource);
		try{
			...
			if(user == null){
				...
				if((user.getUserLevel() & 0x00000001) == 1){
					content = AutoProperties.getString(cmd + ".000001", "您好！您目前订购的业务有：1、彩票投注业务，取消请回复QXA！", resource);
				}
				if((user.getUserLevel() & 0x00000002) == 1){
					content = AutoProperties.getString(cmd + ".000002", "您好！您目前订购的业务有：1、彩票手机报，若取消请回复QXB！", resource);
				}
				if((user.getUserLevel() & 0x00000003) == 3){
					content = AutoProperties.getString(cmd + ".000003", "您好！您目前订购的业务有：1、彩票投注业务，取消请回复QXA；2、彩票手机报，若取消请回复QXB；全部取消请回复00000！", resource);
				}
			}
		}catch(Exception e){
