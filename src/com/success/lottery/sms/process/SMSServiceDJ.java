 ...
 *
 */
public class SMSServiceDJ extends SMSService{

	private Log logger = LogFactory.getLog(SMSServiceDJ.class.getName());
		...
		try{
			...
			if(!user.isBindMobileFlag()){
				...
				if(parameters.length != 3){
					throw new LotteryException(880001, "修改信息参数错误");
				}
				String cardNo = parameters[1].trim();
				String realName = parameters[2].trim();
				
				int rc = accountService.updateUserdetailInfo(user.getNickName(), user.getUserId(), realName, user.getAddress(), cardNo, user.getEmail());
				if(rc != 1){
					throw new LotteryException(880002, "修改信息未能成功");
				}
				content = AutoProperties.getString(cmd + ".000000", "恭喜您，投注成功：", resource);
				if(content.indexOf("{realName}") >= 0){
					content = content.replaceAll("\\{realName\\}", realName);
				}
				if(content.indexOf("{cardNo}") >= 0){
					content = content.replaceAll("\\{cardNo\\}", cardNo);
				}
			}
