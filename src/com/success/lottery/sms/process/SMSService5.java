...
/**
 * 调用process完成排列五机选单式一注的投注，返回投注结果
 * 返回的字符串为sms.properties中配置的命令
 * @author bing.li
 *
 */
public class SMSService5 extends SMSService{
	...
	public String process(String cmd, MO mo){
		...
		try{
			...
			if(!user.isBindMobileFlag()){
				...
				String betCode = LotteryTools.lotteryRandomBetCode(1000004);
				BetServiceInterf betService = ApplicationContextUtils.getService("busLotteryBetService", BetServiceInterf.class);
				String betResult = betService.betSms(user.getUserId(), 1000004, 0, 0, 1, betCode);
