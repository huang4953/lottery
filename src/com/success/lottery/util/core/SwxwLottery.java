/**
 * Title: SwxwLottery.java
 * @Package com.success.lottery.util.core
 * Description: TODO(用一句话描述该文件做什么)
 * @author gaoboqin
 * @date 2010-12-3 上午11:44:36
 * @version V1.0
 */
package com.success.lottery.util.core;
 ...
 * 江西15选5
 * @author gaoboqin
 * 2010-12-3 上午11:44:36
 * 
 */

public class SwxwLottery extends LotteryAbstractTool implements LotteryInterf {
	...
	public boolean checkBetType(String playType, String betType,
		...
		if (SWXW_PLAYTYPE_NO_ADDITIONAL.equals(playType)) {
	...
	private boolean checkBet(String betType,String betString) throws LotteryUnDefineException,Exception{
		if(SWXW_BETTYPE_SINGLE.equals(betType)){
			return checkBase(SWXW_BET_0_REGULAR,betString);
		}else if(SWXW_BETTYPE_DUPLEX.equals(betType)){
			return checkBase(SWXW_BET_1_REGULAR,betString);
		}else if(SWXW_BETTYPE_DANTUO.equals(betType)){
	 ...
	 * @return
	 */
	private boolean checkBase(String regular,String betString){
		boolean checkResult = true;
		String [] betGroupByNum = splitGroup(betString,SWXW_BET_GROUP_SEPARATOR,true);
		for(String group : betGroupByNum){
			if(!checkRules(regular,group)){
				checkResult = false;
				break;
			}else{
	...
	private boolean checkDanTuo(String betString){
		...
		String [] betGroupByNum = splitGroup(betString,SWXW_BET_GROUP_SEPARATOR,true);
		for(String group : betGroupByNum){
			if(!checkRules(SWXW_BET_2_REGULAR,group)){//格式不合格
				checkResult = false;
				break;
			}else if(!checkDanTuoRepeat(group,2)){//校验是否重复,包括前区和后区
				checkResult = false;
				break;
			}else if(!checkDanTuoTotalNum(group)){//校验总数是否够5个
				checkResult = false;
				break;
			}
		}
